...
Rules: real brand/generic names, rating 4.0–4.9, NO explanation."""
    ...
    try:
        ...
        for p in products:
            ...
            # Always enforce real search URL — never use Claude-invented product URLs
...
async def search_all_products(query: str = Query(..., description="FSA product search query")):
    ...
    # ── 2. Amazon scraper + other retailer search run in parallel ─────────
    DEFAULT_FLOOR = 12.99
    ...
    amazon_products, other_products = await asyncio.gather(scraper_task, other_task)

    # ── 3. Enforce real price floor ────────────────────────────────────────
